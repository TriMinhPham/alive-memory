...
class AliveMemorySystem(MemorySystemAdapter):
    ...
    async def setup(self, config: dict) -> None:
        ...
        # Consolidation: group by session instead of per-turn reflection.
        # Per-turn reflection is designed for incremental use (few turns/day).
        # Benchmark dumps ~550 turns at once — session grouping gives the LLM
        # full conversation context for better fact extraction (~53 calls vs ~174).
        consol = sdk_config.setdefault("consolidation", {})
        consol.setdefault("group_by_session", True)
